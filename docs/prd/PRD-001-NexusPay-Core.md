...
**用户故事：**
作为 NexusPay Core，当收到 Quote 时，我需要根据商户配置和交易特征自动选择最合适的支付模式。
...
Relayer 作为 NexusPay Core 的子模块，包含三个核心组件：
...
**用户故事：**
作为 NexusPay Core，我需要持续监听 PlatON 链上的 USDC Transfer 事件和 NexusPayEscrow 合约事件，确保所有支付状态变更都被及时捕获。
...
- Escrow 模式：过滤 NexusPayEscrow 合约 event logs
- PlatON 出块时间约 1 秒，3 秒间隔足够及时

超时退款自动触发 (Escrow 模式)：
- 每 60 秒扫描 status = 'ESCROWED' 且 release_deadline 已过的订单
- 通过 Relayer 调用 NexusPayEscrow.refund(paymentId)
- Gas 由 Relayer 承担
```
...
NexusPay Core 支持两种支付模式，共享前半段状态，后半段根据模式分叉。
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  escrow_contract     TEXT,                   -- NexusPayEscrow 合约地址
...
```json
{
  ...
  "escrow_instruction": {
    ...
    "chain_name": "PlatON",
    "payment_method": "ESCROW_CONTRACT",
    "escrow_contract": "0xNexusPayEscrowAddress",
    "token_address": "0xPlatON_USDC_Address",
    "amount_uint256": "530000000",
    "amount_display": "530.00",
    "eip3009_sign_data": {
...
```
User Agent (UA)       NexusPay Core        Relayer         NexusPayEscrow       Merchant Agent (MA)
...
│   │   │   ├── tx-tracker.ts        # 交易追踪
│   │   │   ├── usdc.ts             # USDC 合约交互
│   │   │   └── escrow-contract.ts   # v1.1: NexusPayEscrow 合约交互
│   │   │
...
├── src/
│   └── NexusPayEscrow.sol           # Escrow 合约
├── test/
│   └── NexusPayEscrow.t.sol         # 合约测试
├── script/
│   └── Deploy.s.sol                 # 部署脚本
└── foundry.toml
```
...
| 智能合约 | Solidity + Foundry | NexusPayEscrow 合约开发与测试 |
...
- [ ] NexusPayEscrow.sol 合约实现 (EIP-3009 + 担保状态机)
