# RFC-010: xXAgent Pay Escrow 智能合约规范

| 元数据 | 内容 |
| --- | --- |
| **标题** | xXAgent Pay Escrow 智能合约规范 |
| **版本** | 2.0.0 |
| **状态** | Standards Track (Draft) |
| **作者** | Cipher & XAgent Pay Architect Team |
| **创建日期** | 2026-02-24 |
| **更新日期** | 2026-02-26 |
| **依赖** | RFC-005v3 (Payment Core MVP), RFC-009 (Webhook Standard), RFC-001 (DID) |
| **目标链** | XLayer Devnet (chain_id: 20250407) |
...
本 RFC 定义 xXAgent Pay Escrow 智能合约的完整规范。合约作为支付担保人，实现"用户付款 -> 资金锁定 -> 商户履约 -> 资金释放"的担保交易流程。与 RFC-005v2 的 Direct Transfer 模式互补，为高价值交易、跨链支付和纠纷仲裁提供链上安全保障。
...
> 注意：Escrow 模式下用户不需要持有任何 LAT (XLayer 原生代币)，所有链上交易费用由 Relayer 承担，成本从协议手续费 (0.3%) 中覆盖。
...
XLayer 链上的 USDC 支持 EIP-3009 (`transferWithAuthorization`)，其接口如下：
 ...
 * @notice 基于 USDC 的担保支付合约，支持 xXAgent Pay 支付协议
 ...
 * - 初始仲裁人 = XAgent Pay operator：部署时自动设置，后续可更换
...
contract xXAgent PayEscrow is ReentrancyGuard, Ownable {
    ...
    struct EscrowRecord {
        bytes32 paymentId;           // xXAgent Pay 支付 ID (keccak256)
    ...
    /**
     * @notice 部署 xXAgent PayEscrow 合约
     ...
     * @param _nexusOperator         XAgent Pay 管理钱包 (初始仲裁人 + 操作员)
     */
    constructor(
        address _usdc,
        ...
        // 初始设置: XAgent Pay operator 同时作为仲裁人和 Core 操作员
     ...
     *
     * @param _paymentId     xXAgent Pay 支付 ID (bytes32, Core 生成)
    ...
    /**
     * @notice 仲裁人裁决争议
     * @dev 仅仲裁人可调用 (初始阶段为 XAgent Pay 管理钱包)
...
### 4.6 合约部署参数 (XLayer 主网)

| 参数 | 建议值 | 说明 |
| --- | --- | --- |
| `_usdc` | XLayer USDC 合约地址 | 必须支持 EIP-3009 (transferWithAuthorization) |
...
| `_protocolFeeRecipient` | XAgent Pay 多签钱包 | 手续费收款地址 |
| `_nexusOperator` | XAgent Pay 管理钱包 | 初始仲裁人 + Core 操作员 + Relayer 地址 |
...
2. 对每个过期订单：
   a. 通过 Relayer 调用 xXAgent PayEscrow.refund(paymentId) (Relayer 承担 Gas)
   b. 如果交易成功：
...
**v1.1 决策：初始仲裁人 = XAgent Pay 管理钱包 (nexusOperator)**

初始阶段，XAgent Pay 管理钱包同时承担 operator (release 权限) 和 arbiter (dispute resolve 权限) 两个角色。这简化了部署和运营，后续可通过 `setArbiter()` 更换为独立的仲裁人地址。

| 规则 | 说明 |
| --- | --- |
| **初始仲裁人** | 合约 constructor 自动设置 `arbiter = nexusOperator` (XAgent Pay 管理钱包) |
...
轮询策略:
- 每 3 秒查询 xXAgent PayEscrow 合约的新事件
...
src/contracts/
├── src/
│   ├── xXAgent PayEscrow.sol        # 主合约 (含 depositWithAuthorization)
│   └── interfaces/
│       └── IERC3009.sol           # EIP-3009 接口定义
│
├── test/
│   ├── xXAgent PayEscrow.t.sol      # Foundry 单元测试
│   ├── xXAgent PayEscrow.gas.t.sol  # Gas 消耗测试
│   └── mocks/
...
| XAgent Pay 字段 | ISO 20022 Element | 说明 |
| --- | --- | --- |
...
| **时间操纵** | 矿工/验证者操纵 block.timestamp | XLayer 出块时间稳定 (~1s)；超时窗口设置为小时级别 |
...
XLayer 链上的 USDC 支持 EIP-3009 (`transferWithAuthorization`)，而非 EIP-2612 (Permit)。EIP-3009 的核心优势在于：
...
| **XLayer USDC 支持** | 不支持 | 支持 |

因此，我们采用 EIP-3009 + Relayer 代付方案，实现用户完全零 Gas 的支付体验。

### 11.2 Relayer 服务架构

Relayer 作为 xXAgent Pay Core 的一个子模块，负责代用户提交链上交易并支付 Gas。
...
4. 如果余额 < CRITICAL_THRESHOLD:
   a. 发送紧急告警
   b. 触发自动充值流程 (从 XAgent Pay 运营钱包转入 LAT)
...
- [ ] 实现 IERC3009 接口定义
- [ ] 实现 xXAgent PayEscrow.sol 核心合约 (含 depositWithAuthorization)
...
#### Phase 4: XLayer 主网部署 (1 周)

- [ ] AI 安全审计最终轮 + Foundry fuzz/invariant 测试
- [ ] XLayer 主网部署 (设置 nexusOperator 为初始 arbiter + operator)
...
6. **链上 DID 注册表**：XAgent PayMerchantRegistry 合约集成
...
| 编号 | 议题 | 决策 | 影响 |
| --- | --- | --- | --- |
| 1 | XLayer USDC 是否支持 EIP-2612 Permit | **不支持 Permit，支持 EIP-3009 (transferWithAuthorization)** | 合约改用 depositWithAuthorization，取消 depositWithPermit |
| 2 | Gas 费由谁承担 | **Relayer 代付，Gas 成本从协议手续费 (0.3%) 中覆盖** | 新增 Relayer 服务模块；用户无需持有 LAT |
| 3 | 仲裁人选拔标准 | **初始阶段 arbiter = nexusOperator (XAgent Pay 管理钱包)，后续可通过 setArbiter() 更换** | 合约 constructor 自动设置；简化部署 |
...
2. **跨链 Escrow 架构**：是在源链 deposit、目标链 release，还是统一在 XLayer Hub？
...
以下记录 RFC-010 v2.0.0 与实际部署的 xXAgent PayEscrow v4.0.0 合约的对齐变更。
...
**关键发现**: XLayer Devnet EVM 的 `block.timestamp` 使用**毫秒**而非秒。
...
- 必须使用 `--legacy` flag (EIP-1559 交易在 XLayer 上 gas price 为 1 wei，太低)
- 使用 `--with-gas-price 20000000000` (20 gwei，网络最低 ~10 gwei)
- XLayer 不支持交易替换，使用 `--legacy` 以避免 pending 交易问题
