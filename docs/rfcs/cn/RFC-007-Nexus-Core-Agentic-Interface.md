...
XAgent Pay Core 作为一个 **MCP Server** 运行，向网络中的 Agent 提供两套标准的插件（Plugins）：
...
此插件赋予 MA 跨链无感的收款能力，MA 只需要关注 Hub Chain (XLayer) 的状态。
### 🛠️ Tool A: `verify_order_lock` (验证发货条件)
**生命周期阶段:** **Fulfillment** (MA 准备发货前)
**功能:** 查询 Hub Chain (XLayer) 上的 Escrow 合约，确认资金是否安全锁定 (LOCKED)。
...
**功能:** 从 XAgent Pay Core 拉取用户的签名，并在 XLayer 链上调用合约提取资金。
* **Input Schema:**
```json
{ "nexus_payment_id": "NEX-UUID-001" }
```
...
XAgent Pay Core 维护的单一真实状态源，通过 MCP Resource `nexus://core/orders/{id}` 暴露。
...
| **LOCKED** | **[里程碑]** XLayer 已锁定 | Hub 合约事件 | ✅ "Payment Successful" |
...
1. **路由引擎:** 需要硬编码或配置 XLayer, Base, Ethereum 的 RPC 节点，实时拉取 Gas Price 计算 `est_gas_fee_usd`。
2. **MPC 服务:** 集成 MPC 钱包 SDK（如 Fireblocks, Coinbase WaaS 或自研 TSS），实现 `finalize_payment` 时派生子地址。
3. **KYT 钩子:** 在 `DETECTING` 状态时，阻塞流程，调用 KYT API，仅在通过后执行 `createVirtualDeposit` 到 XLayer。
