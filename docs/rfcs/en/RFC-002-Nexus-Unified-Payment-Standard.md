...
1. **Quote Phase (Merchant Side):** The merchant generates a "quote" containing the business order number and ISO metadata. **The merchant does not generate a XAgent Pay ID, nor does it generate on-chain Calldata.**
2. **Orchestration Phase (Core Side):** XAgent Pay Core receives the quote, assigns a `nexus_payment_id`, resolves DID routing, and generates settlement transactions compliant with ISO standards.
...
When the User Agent executes an aggregated payment (`nexus/orchestrateBatch`), XAgent Pay Core returns this object. This is the content that the User Agent ultimately signs.
### 3.1 JSON Schema
```json
{
...
// [ID Bridge] XAgent Payment ID <---> Merchant Order Ref
...
| XAgent Pay JSON Field | ISO 20022 XML Tag | Business Meaning |
| --- | --- | --- |
...
```
### 6.2 DID Resolution
XAgent Pay Core **must never** trust the recipient address passed from the frontend. It must use `merchant_did` to call the on-chain Registry contract (or a trusted cache) to obtain the authentic `payment_address`.
---