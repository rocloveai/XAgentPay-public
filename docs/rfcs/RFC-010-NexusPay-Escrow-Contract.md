...
| **创建日期** | 2026-02-24 |
| **更新日期** | 2026-02-24 |
| **依赖** | RFC-005v2 (Payment Core MVP), RFC-009 (Webhook Standard), RFC-001 (DID) |
...
| `DISPUTED` | 4 | 用户发起争议，等待仲裁 | RESOLVED_TO_MERCHANT, RESOLVED_TO_PAYER |
...
MVP 阶段不使用可升级代理模式 (UUPS/Transparent Proxy)，原因：

1. 降低合约复杂度，减少攻击面
2. MVP 阶段交易量有限，可部署新合约迁移
3. 未结算的 Escrow 在旧合约中自然结束

后续版本考虑引入 EIP-1967 Transparent Proxy 实现无缝升级。
