...
export async function handleRestApiRequest(
  ...
  if (paymentByIdMatch && req.method === "GET") {
    return handlePaymentStatus(deps, res, {
      xagentPaymentId: paymentByIdMatch[1],
    });
  }

  // --- GET /api/payments?group_id=...&merchant_order_ref=...&xagent_payment_id=... ---
  if (url.pathname === "/api/payments" && req.method === "GET") {
    ...
    const paymentId = url.searchParams.get("xagent_payment_id") ?? undefined;
    ...
    if (!groupId && !orderRef && !paymentId) {
      jsonResponse(res, 400, {
        error: {
          code: "MISSING_PARAMS",
          message:
            "Provide at least one of: group_id, merchant_order_ref, xagent_payment_id",
        },
      });
      return true;
    }
...
async function handlePaymentStatus(
  ...
  try {
    const result = await deps.orchestrator.getPaymentStatus({
      xagentPaymentId: params.xagentPaymentId,
    ...
    const payment = result.payment
      ? {
          xagent_payment_id: result.payment.xagent_payment_id,
...
async function handleACPSubmitDeliverable(
  ...
  try {
    ...
    const { xagent_payment_id, merchant_did, deliverable } = body;

    if (!xagent_payment_id || !merchant_did || !deliverable) {
      jsonResponse(res, 400, {
        error: {
          code: "MISSING_PARAMS",
          message:
            "Required fields: xagent_payment_id, merchant_did, deliverable",
        },
      });
      return true;
    }
    ...
    // Find the payment
    const payment = await deps.paymentRepo.findById(xagent_payment_id);
    if (!payment) {
      jsonResponse(res, 404, {
    ...
    await deps.paymentRepo.updateStatus(xagent_payment_id, "JOB_FUNDED", {
