...
describe("Checkout", () => {
  ...
  beforeEach(async () => {
    ...
    await groupRepo.updateInstruction(GROUP_ID, {
      ...
      xagent_group_sig: "0x" + "ee".repeat(65),
  ...
  describe("GET /checkout/:groupId", () => {
    ...
    it("includes group signature validation in rendered HTML", async () => {
      ...
      expect(res.body).toContain("xagent_group_sig");
  ...
  describe("GET /api/checkout/:groupId", () => {
    ...
    it("returns instruction with xagent_group_sig and core_operator_address", async () => {
      const { req, url } = makeReq("GET", `/api/checkout/${GROUP_ID}`);
      const res = makeRes();
      await handleCheckoutRequest(
        deps,
      ...
      const data = JSON.parse(res.body);
      expect(data.instruction.xagent_group_sig).toBeDefined();
      expect(data.instruction.xagent_group_sig).toMatch(/^0x/);
