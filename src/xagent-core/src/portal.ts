  ...
  if (detail.classList.contains("hidden")) {
    ...
    try {
      ...
      for (var p of payments) {
        html += '<tr class="border-b border-slate-700/30 cursor-pointer hover:bg-slate-700/20 transition-colors" onclick="event.stopPropagation();showDetail(\\'' + esc(p.xagent_payment_id) + '\\')">' +
          '<td class="px-3 py-2 font-mono text-xs text-slate-300">' + esc(p.xagent_payment_id) + '</td>' +
...
function updatePayments(payments) {
  ...
  var html = "";
  for (var p of payments) {
    html += '<tr class="border-b border-slate-700/50 cursor-pointer hover:bg-slate-700/30 transition-colors" onclick="showDetail(\\'' + esc(p.xagent_payment_id) + '\\')">' +
      '<td class="px-4 py-3 font-mono text-xs text-slate-300">' + esc(p.xagent_payment_id) + '</td>' +
...
async function showDetail(id) {
  ...
  try {
    ...
    html += '<div class="flex justify-between"><span class="text-slate-500">ID</span><span class="font-mono text-xs text-slate-300">' + esc(p.xagent_payment_id) + '</span></div>';
