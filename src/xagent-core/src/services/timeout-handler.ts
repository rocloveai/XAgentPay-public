...
export class TimeoutHandler {
  ...
  async sweepOnce(): Promise<void> {
    ...
    if (this.webhookNotifier && expiredPayments.length > 0) {
      for (const payment of expiredPayments) {
        this.webhookNotifier
          ...
          .catch((err) =>
            console.error(
              `[TimeoutHandler] Webhook failed for ${payment.xagent_payment_id}:`,
    ...
    for (const payment of expiredEscrowed) {
      if (!payment.payment_id_bytes32) {
        console.error(
          `[TimeoutHandler] ESCROWED payment ${payment.xagent_payment_id} has no payment_id_bytes32, skipping`,
        );
        continue;
      }
      ...
      try {
        ...
        console.error(
          `[TimeoutHandler] Refund submission failed for ${payment.xagent_payment_id}:`,
          err instanceof Error ? err.message : err,
        );
    ...
    for (const payment of expiredDisputes) {
      ...
      try {
        ...
        console.error(
          `[TimeoutHandler] Auto-resolve failed for ${payment.xagent_payment_id}:`,
          err instanceof Error ? err.message : err,
        );
        await this.syncFromChainState(payment);
      }
  ...
  private async syncFromChainState(payment: PaymentRecord): Promise<void> {
    try {
      ...
      const id = payment.xagent_payment_id;
      ...
      console.error(
        `[TimeoutHandler] Chain state sync failed for ${payment.xagent_payment_id}:`,
