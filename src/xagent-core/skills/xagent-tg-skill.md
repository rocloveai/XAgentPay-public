...
After you call `xagent_orchestrate_payment` and receive a `checkout_url` + `group_id`, POST the order details to this service to render a beautiful order card in the user's Telegram chat. The card includes:
...
**After calling `xagent_orchestrate_payment`, you MUST follow these rules:**
...
5. The same rule applies to `xagent_get_payment_status` — do not dump raw status JSON. Instead, summarize naturally: "Your payment has been confirmed!" or "Still waiting for on-chain confirmation."
...
3. You call xagent_orchestrate_payment → get checkout_url, group_id, payments
...
After calling `xagent_orchestrate_payment`, you get back a response like:
...
| `XAGENT_CORE_URL` | Yes | — | xXAgent Pay Core URL for status polling |
