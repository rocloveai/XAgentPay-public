...
import { updateStatus, storeQR } from "./order-store.js";
...
// ---------------------------------------------------------------------------
// Event handling — KEY DIFFERENCE: generates QR code on PAID
// ---------------------------------------------------------------------------
...
export async function handleWebhookEvent(
  ...
  if (newStatus) {
    ...
    if (updated) {
      ...
      // eSIM-specific: generate QR code when payment is confirmed
      if (newStatus === "PAID") {
        try {
          const { qr_data_url, activation_code } = await generateEsimQR(
            data.merchant_order_ref,
          );
          storeQR(data.merchant_order_ref, qr_data_url, activation_code);
          console.error(
            `[Webhook] eSIM QR generated for ${data.merchant_order_ref}: ${activation_code}`,
          );
        } catch (err) {
          console.error(
            `[Webhook] Failed to generate eSIM QR for ${data.merchant_order_ref}:`,
            err,
          );
        }
      }
