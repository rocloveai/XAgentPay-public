...
// --- API Configuration ---
const API_URL = import.meta.env.VITE_XAGENT_CORE_URL || "https://api.xagenpay.com";
...
const MarketPage = ({ lang, initialTab = 'discover' }: { lang: Language; initialTab?: 'discover' | 'list' }) => {
  ...
  const [starredDids, setStarredDids] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(localStorage.getItem('xagent_starred') || '[]')); }
  ...
  const toggleStar = async (did: string) => {
    ...
    try {
      ...
      setStarredDids(prev => {
        ...
        localStorage.setItem('xagent_starred', JSON.stringify([...next]));
        return next;
      });
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredAgents.map((agent) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-black/5 dark:border-white/5 transition-colors mt-auto">
                          <div className="flex flex-col">
