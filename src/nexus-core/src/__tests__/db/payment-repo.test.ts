...
describe("MockPaymentRepository", () => {
  ...
  it("insert returns a full record with CREATED status", async () => {
    ...
    expect(record.nexus_payment_id).toBe("NEX-001");
  ...
  it("findById returns inserted record", async () => {
    ...
    expect(found!.nexus_payment_id).toBe("NEX-001");
  });
  ...
  it("findExpiredAwaiting filters correctly", async () => {
    ...
    await repo.insert(
      makeParams({ nexus_payment_id: "NEX-EXP", expires_at: pastExpiry }),
    );
    await repo.insert(
      makeParams({
        nexus_payment_id: "NEX-FUTURE",
        quote_hash: "0xhash2",
    ...
    expect(expired[0].nexus_payment_id).toBe("NEX-EXP");
  });

  it("findExpiredEscrowed filters by release_deadline", async () => {
    const past = new Date(Date.now() - 60_000).toISOString();
    await repo.insert(makeParams({ nexus_payment_id: "NEX-ESC" }));
    ...
    expect(results[0].nexus_payment_id).toBe("NEX-ESC");
  });
});
